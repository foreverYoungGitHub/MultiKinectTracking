...
BOOL KinectOperation::Calibration_run(CameraSpacePoint *pAns)
{
	...
	for (INT i1 = nTopYLeft; i1 < nTopYRight; i1++)
	{
		printf("%f ", (DOUBLE)*(pDepthBufferSum + nTopX*nFrameWidth + i1));
	}
	printf("\n");
...
BOOL KinectOperation::KinectGetDepthFrame(INT *nFrameHeight, INT *nFrameWidth, UINT16 **pBuffer, INT **pStateBuffer)
{
	...
	nMeanDepth = nMeanDepth / (nMarkEn - nMarkSt + 1) + MARK_RADIUS + MARK_THRESHOLD_DEC;
	printf("[%d %d] with Threshold %d and Line %d, meandepth= %d\n", nMarkSt, nMarkEn, nCurrentMarkThresHold, nMarkPos,nMeanDepth);
...
void KinectOperation::JointShade(Joint *pCurrent, INT *nShade)
{
	...
	*/
	for (INT i1 = 0; i1 < BODY_LENGTH; i1++)
	{
		nShade[i1] = pCurrent[i1].TrackingState;
	}
}
